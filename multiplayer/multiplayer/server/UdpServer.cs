...
namespace multiplayer.server
{
	...
	class UdpServer
	{
		...
		public void Start(IPEndPoint ep)
		{
			...
			{
				throw new PortAlreadyUsed(ep.Port, e);
			}
			this._endpoint = ep;
			this._running = true;
			this.Receive();
		...
		{
			Console.WriteLine("[DBG]: Message received.");
		...
		public virtual void Receive()
		{
			if (this.Running)
			{
				Console.WriteLine("[DBG]: Start receiving message.");
				UdpServerState state = new UdpServerState(new IPEndPoint(this._endpoint.Address, this._endpoint.Port));
				this._socket.BeginReceiveFrom(this._data, 0, this._data.Length, SocketFlags.None, ref state.Endpoint, this.ReceiveHandler, state);
			}
		}
		...
		{
			this.Send(Encoding.ASCII.GetBytes(data), endpoint);
		}
	}
}
